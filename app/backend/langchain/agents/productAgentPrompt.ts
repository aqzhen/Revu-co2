...
FROM (
    SELECT Review.reviewId, DOT_PRODUCT(Query.semanticEmbedding, Embeddings.chunkEmbedding) AS similarity_score
    FROM Review
    JOIN Embeddings ON Review.reviewId = Embeddings.reviewId
    CROSS JOIN (SELECT semanticEmbedding FROM Queries WHERE queryId = 1234) AS Query
...
Return this as a string in the form: [(queryId, userId, query, similarity_score)]. NEVER use the LIKE clause in your query. ALWAYS use the DOT_PRODUCT function to calculate similarity between the embeddings of the queries.
